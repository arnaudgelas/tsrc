...
class GroupNotFound(GroupError):
    def __init__(self, group_name, parent_group=None):
        ...
        super().__init__("No such group: %s" % group_name)
...
class CircularInclude(GroupError):
    def __init__(self, group_name):
        self.group_name = group_name
        message = "Circular include detected when parsing: %s" % (group_name)
        super().__init__(message)
    ...
    def _rec_get_elements(self, res, group_names, *, parent_group):
        for group_name in group_names:
            if group_name in self._groups_seen:
                raise CircularInclude(group_name)
